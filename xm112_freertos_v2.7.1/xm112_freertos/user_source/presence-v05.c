...
/*
    #define XM11x_SENS_EN_PIN    106 // PD10
  	if (!acc_device_gpio_write(XM11x_SENS_EN_PIN, 0))//acconeer\xm112_freertos\source\acc_boardbase_xm11x.c
	{
...
static void configure_presence(acc_detector_presence_configuration_t presence_configuration)
{
	 ...
	 */
	acc_detector_presence_configuration_service_profile_set(presence_configuration, ACC_SERVICE_PROFILE_5);//ACC_SERVICE_PROFILE_3
	acc_detector_presence_configuration_update_rate_set(presence_configuration, UPDATE_RATE);
	acc_detector_presence_configuration_detection_threshold_set(presence_configuration, 1.8f);//The value 2.0 corresponds to the detection score  2000. We multiply by 1000 to avoid decimals when we print the score values.
	acc_detector_presence_configuration_start_set(presence_configuration, 0.7);//was 0.4
